...
class BandageDeviceActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Берем характеристику поддерживающую Read, Write и Notifiable
        val characteristic = characteristics.first { it.isNotifiable() && it.isReadable() && it.isWritable() }
        Commands.EnableNotification.applyBtAction()
    ...
    private val connectionEventListener by lazy {
        ConnectionEventListener().apply {
            onDisconnect = {
                runOnUiThread {
                    AlertDialog.Builder(this@BandageDeviceActivity)
    ...
    private enum class Commands(hexCommand : String,
    ){
        EnableNotification("",
        ) {
            override fun applyBtAction(device: BluetoothDevice) {
                // Логика для включения
            }
        },
        ReadTemperature ("58"){
            override fun applyBtAction(device: BluetoothDevice) {
                // Логика для оманды с ответом
            }
        };

        abstract fun applyBtAction(device: BluetoothDevice)
    }
